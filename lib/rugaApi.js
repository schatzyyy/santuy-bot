...
const nowm2 = async (url) => new Promise((resolve, reject) => {
	axios.get(`https://api.zeks.xyz/api/tiktok?url=${url}&apikey=apivinz`)
	.then((res) => {
        const gatau = `*Judul :* ${res.data.title}\n*UserName :* ${res.data.author}\n*Total like :* ${res.data.like_count}`
	resolve({links: res.data.no_watermark, text1: gatau})
})
.catch(err => {
	reject(err)
})
 ...
 * @returns {Object}
 */
const its = (username) => new Promise((resolve, reject) => {
    console.log('Searching for IG Story...')
    fetchJson(`https://docs-jojo.herokuapp.com/api/igstory?username=${username}`)
        .then((result) => resolve(result))
        .catch((err) =>  reject(err))
})
 ...
 * @returns {Object}
 */
const sticker = (q) => new Promise((resolve, reject) => {
    console.log('Searching for sticker...')
    fetchJson(`https://api.zeks.xyz/api/searchsticker?apikey=apivinz&q=${q}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
...
const kusonime = async (judul) => new Promise((resolve, reject) => {
    axios.get(`https://lindow-api.herokuapp.com/api/anime/kusonime?search=${judul}&apikey=LindowApi`)
    .then((res) => {
...
const story = async (query) => new Promise((resolve, reject) => {
    axios.get(`https://api.zeks.xyz/api/igs?apikey=apivinz&username=${query}`)
    .then((res) => {
        resolve(res.data.result)
    })
    .catch((err) => {
...
const ymp4 = async (url) => new Promise((resolve, reject) => {
    axios.get(`https://fahmiapi.herokuapp.com/yt/mp4?url=${url}&apikey=freeTrial2k21`)
    .then((res) => {
		resolve(res.data.result)
    })
    .catch((err) =>{
...
const kbbi = async (url) => new Promise((resolve, reject) => {
    axios.get(`https://kocakz.herokuapp.com/api/edu/kbbi?query=${url}`)
    .then((res) => {
        resolve(res.data.result)
    })
    .catch((err) =>{
